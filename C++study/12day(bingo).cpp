...
bool ChangeNumber(int* pArray, int iInput)
{
	...
	//여기까지 오게되면 return false가 동작 안된것이므로 같은 숫자가 없다는 것이다. 즉, 중복된 숫자를 입력했기 때문에  true를 리턴한다.
	return true;
}
...
int BingoCountingH(int* pArray)
{
	...
	for (int i = 0; i < 5; ++i)
	{
		...
		// j for문을 빠져나오고 나면 현재 줄의 가로 별 개수가 몇개인지 iStar1변수에 들어가게 된다.
...
int main()
{
	...
	while (true)
	{
		...
		//중복 입력을 체크하기 위한 변수이다. 기본적으로 중복되었다라고 하기 위해 true로 주었다.
