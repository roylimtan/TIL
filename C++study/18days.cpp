...
//숫자 입력을 받는다 INT_MAX를 리턴하면 입력 오류이다.
int InputInt()
{
	int iInput;
	cin >> iInput;
	...
	if (cin.fail())
	{
		cin.clear();
		cin.ignore(1024, '\n');
		return INT_MAX; //INT_MAX가 리턴되면 잘못됐다는 의미
	}
...
//입력 오류일 경우 혹은 잘못된 번호를 입력한 경우 MM_NONE을 반환해준다.
int OutputMainMenu()
{
	system("cls");
	cout << "*************************** 로비 ***************************" << endl;
...
bool LoadPlayer(_tagPlayer* pPlayer)
{
	...
	if (pFile)
	{
		...
		//만약 저장할때 무기를 차고 있었다면 해당 무기정보도 같이 저장이 되었다. 그러므로 여기서 차고 있을 경우 읽어야한다.
		if (pPlayer->bEquip[EQ_WEAPON])
...
bool SavePlayer(_tagPlayer* pPlayer)
{
	...
	if (pFile)
	{
		...
		//만약 저장할때 무기를 차고 있었다면 해당 무기정보도 같이 저장이 되었다. 그러므로 여기서 차고 있을 경우 저장한다.
		if (pPlayer->bEquip[EQ_ARMOR])
...
_tagMonster CreateMonster(char* pName, int iAttackMin, int iAttacMax, int iArmorMin, int iArmorMax, int iHP, int iMP,
...
EQUIP ComuteEquipType(ITEM_TYPE eType)
{
	EQUIP	eq;
...
void RunInventory(_tagPlayer* pPlayer)
{
	while (true)
	{
		...
		// 결정하게 한다.
		EQUIP	eq = ComuteEquipType(pPlayer->tInventory.tItem[idx].eType);
...
int main()
{
	...
	g_tLvUpTable[JOB_WIZARD - 1] = CreateLvUPStatus(15, 20, 3, 7, 20, 40, 50, 10);
