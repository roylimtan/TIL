...
링크드리스트는 데이터를 저장하기 위한 노드라는 것이 존재한다. 각 노드의 특징은 다음 노드를 알 수 있게 된다.
(다음노드의 메모리 주소를 저장한다는 의미이다.)
...
{
	/*
	포인터는 가급적이면 초기화할때 NULL(0)로 초기화해두고 쓰는것이 좋다.
...
void Insert(PLIST pList)
{
	...
	//그래서 다음 노드는 NULL로 초기화하고 정보는 위에서 입력받은 학생정보를 주도록 한다.
	pNode->pNext = NULL;
...
void Delete(PLIST pList)
{
	...
	while (pNode != NULL)
	{
		if (strcmp(pNode->tStudent.strName, strName) == 0)
		{
			...
			//이전 노드가 있을 경우에는 이전 노드의 다음을 지운 노드의 다음노드로 연결해준다.
			else
			{
				delete pNode;
				pPrev->pNext = pNext;
